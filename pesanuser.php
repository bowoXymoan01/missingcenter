...
$pesan = query("SELECT id, nama, email,nim,telepon, pesan, CONCAT(DATE_FORMAT(waktu, '%Y-%m-%d'),DAYNAME(waktu), ' ', DATE_FORMAT(waktu, '%H:%i:%s')) AS waktu FROM kontak_kami" );
...
<body>
    ...
    <table  style="padding: 5px; border: 1px solid black;" border="1" cellpadding="20" cellspacing="0" class="table table-bordered table-striped table-hover">
        ...
        <tr>
            ...
            <td><?= $row["pesan"]; ?></td> <!--merek-->
            <td><?= $row["waktu"]; ?><br></td> <!--deskripsi-->
        </tr>
        <?php $i++; ?>
        <?php endforeach; ?>
    </table>
