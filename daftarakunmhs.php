<?php
    ...
    $akunmhs = query("SELECT * FROM usermhs" );
...
<html lang="en">
    ...
    <body>
        ...
        <table style="padding: 5px; border: 1px solid black;" border="1" cellpadding="10" cellspacing="0" class="table table-bordered table-striped table-hover">
            ...
            <?php $i = 1; ?>
            <?php foreach( $akunmhs as $row ) : ?>
            <tr>
                <td><?= $i ?></td> <!--no-->
                <td><img src="fotoprofil/<?= $row["gambar"];?>" width="150"></td> <!--gambar-->
                <td><?= $row["nama_lengkap"]; ?></td> <!--nama penemu-->
                <td><?= $row["nim"]; ?></td> <!--nama barang-->
                <td>
                    <?= $row["no_wa"]; ?><br>
                    <a href="#" onclick="redirWhatsapp(<?= $row['no_wa']; ?>)" class="btn" target="_blank">Hubungi</a>
                </td> <!--telepon-->
                <td><?= $row["username"]; ?></td> <!--merek-->
                <td><?= $row["password"]; ?></td> <!--deskripsi-->
            </tr>
            <?php $i++; ?>
            <?php endforeach; ?>
        </table>
